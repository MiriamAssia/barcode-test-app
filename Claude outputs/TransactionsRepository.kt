...
class TransactionsRepository {
    ...
    /**
     * כל תנועות ההוצאה בטווח זמן, לדוח הצריכה.
     *
     * הגבולות הם חותמות זמן ISO ב-UTC: from כולל, to לא כולל. כך יום שלם
     * נכנס בדיוק פעם אחת גם כשהטווח נבחר לפי ימי לוח מקומיים.
     *
     * שימו לב: הכמות ב-DB תמיד חיובית גם ב-'out' (הכיוון בשדה type), ולכן
     * הסכימה כאן היא סכום פשוט ולא סכום ערכים מוחלטים.
     */
    suspend fun getOutTransactionsBetween(
        fromInclusive: String,
        toExclusive: String
    ): Resource<List<InventoryTransaction>> = try {
        val rows = postgrest.from("transactions")
            .select {
                filter {
                    eq("type", "out")
                    gte("performed_at", fromInclusive)
                    lt("performed_at", toExclusive)
                }
            }
            .decodeList<InventoryTransaction>()
        Resource.Success(rows)
